// Network
resource "yandex_vpc_network" "dip" {
  name = "dip"
}
...
resource "yandex_vpc_route_table" "route_table" {
  network_id = yandex_vpc_network.dip.id
  static_route {
...
resource "yandex_vpc_security_group" "jump-sg" {
  name       = "jump-sg"
  network_id = yandex_vpc_network.dip.id
  ingress {
    protocol       = "TCP"
...
resource "yandex_vpc_security_group" "elastic-sg" {
  ...
  ingress {
    protocol          = "TCP"
    description       = "Rule for jump ssh"
    security_group_id = yandex_vpc_security_group.jump-sg.id
    port              = 22
  }
